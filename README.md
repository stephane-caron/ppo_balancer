# PPO balancer

This repository has moved to Codeberg:

<p align="center">
  <a href="https://codeberg.org/upkie/ppo_balancer">
    <img src="https://codeberg.org/Codeberg/GetItOnCodeberg/raw/branch/main/get-it-on-blue-on-white.png" alt="Get it on Codeberg" height="80">
  </a>
</p>

Its new address is: **https://codeberg.org/upkie/ppo_balancer**

Codeberg is a free and open-source code forge maintained by a non-profit organization. Development, issues and pull requests now happen over there, and this repository is not mirrored.

## For users

You can clone the new repository and run it as before:

```bash
git clone https://codeberg.org/upkie/ppo_balancer.git
./start_simulation.sh
pixi run agent
```

## For contributors

If you have a local clone of this repository, you can point it at the new remote:

```bash
git remote set-url origin https://codeberg.org/upkie/ppo_balancer.git
```

Issues and pull requests are welcome on [Codeberg](https://codeberg.org/upkie/ppo_balancer). This GitHub repository is archived and no longer receives updates.
